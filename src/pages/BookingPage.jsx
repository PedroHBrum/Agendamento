...
export default function BookingPage({ onDone }) {
  ...
  function genSlots(open,close,dur){
    const s=[],[oh,om]=open.split(':').map(Number),[ch,cm]=close.split(':').map(Number)
  ...
  function renderCalendar(){
    ...
    // Meia-noite de hoje — dias anteriores são passados
    ...
    for(let d=1;d<=dim;d++){
      ...
      // Bloqueia datas ANTERIORES a hoje (hoje está disponível)
      const isPast = dd < todayMidnight
      const isOff=isDayOff(ds), isSel=sel.date===ds
      const isToday=dd.toDateString()===today.toDateString()
      const disabled=isPast||isOff
      cells.push(
        <div key={d}
          className={`cal-day${disabled?' disabled':''}${isSel?' selected':''}${isToday&&!isSel?' today':''}`}
          title={isOff&&!isPast?'Fechado':isPast?'Data passada':''}
          onClick={()=>!disabled&&setSel(s=>({...s,date:ds,time:null}))}>
          {d}
        </div>
      )
    }
  ...
  if(done) return (
    <div className="success-wrap page">
      <div className="success-icon-wrap"><i className="ti ti-check" aria-hidden="true"/></div>
  ...
  return (
    <div className="page">
      ...
      {/* PASSO 1 */}
      {step===1&&<>
        <div className="page-header"><h2>Com quem?</h2>
            ...
            <div key={p.id} className={`card card-interactive${sel.prof?.id===p.id?' selected':''}`}
              ...
              {p.photo_url
                ? <img src={p.photo_url} alt={p.name} onError={e=>e.target.style.display='none'}
                    style={{width:52,height:52,borderRadius:'50%',objectFit:'cover',margin:'0 auto 8px',display:'block',border:`2px solid ${sel.prof?.id===p.id?'var(--primary-600)':'var(--gray-100)'}`}}/>
                : <div style={{width:52,height:52,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:500,fontSize:16,margin:'0 auto 8px',border:`2px solid ${sel.prof?.id===p.id?'var(--primary-600)':'transparent'}`}} className={AV[p.color]||'av-blue'}>{p.initials}</div>
              }
              ...
              {p.bio&&<div style={{fontSize:10,color:'var(--gray-500)',marginTop:3,lineHeight:1.3}}>{p.bio}</div>}
            </div>
          ))}
      ...
      {/* PASSO 2 */}
      {step===2&&<>
        <button className="back-btn" onClick={()=>setStep(1)}><i className="ti ti-arrow-left" aria-hidden="true"/> Profissional</button>
        <div className="page-header"><h2>Qual serviço?</h2>
      ...
      {/* PASSO 3 */}
      {step===3&&<>
        <button className="back-btn" onClick={()=>setStep(2)}><i className="ti ti-arrow-left" aria-hidden="true"/> Serviço</button>
        <div className="page-header"><h2>Quando?</h2>
          <div className="steps" style={{marginTop:8}}>
        ...
        <div className="cal-header">
          <button className="cal-nav-btn" onClick={()=>changeMonth(-1)}><i className="ti ti-chevron-left" aria-hidden="true"/></button>
          <span style={{fontWeight:500,fontSize:14}}>{MONTHS[calMonth]} {calYear}</span>
          <button className="cal-nav-btn" onClick={()=>changeMonth(1)}><i className="ti ti-chevron-right" aria-hidden="true"/></button>
        </div>
        ...
        {sel.date&&availableSlots.length===0&&<p style={{fontSize:13,color:'var(--gray-500)',textAlign:'center',marginTop:20}}>Nenhum horário disponível neste dia.</p>}
        {sel.date&&availableSlots.length>0&&<>
          <p className="section-title">
            Horários disponíveis
            <span style={{fontSize:11,fontWeight:400,textTransform:'none',letterSpacing:0,marginLeft:8}}>
      ...
      {/* PASSO 4 */}
      {step===4&&<>
        <button className="back-btn" onClick={()=>setStep(3)}><i className="ti ti-arrow-left" aria-hidden="true"/> Data e horário</button>
        <div className="page-header"><h2>Confirmar</h2>
          <div className="steps" style={{marginTop:8}}>
        ...
        <div className="summary-box" style={{marginBottom:16}}>
          {[['Profissional',sel.prof?.name],['Serviço',sel.service?.name],['Duração',sel.service?.duration_min+' min'],
            ['Data',(()=>{const d=new Date(sel.date+'T12:00:00');return `${d.getDate()} de ${MONTHS[d.getMonth()]}`})()],['Horário',sel.time]
          ].map(([l,v])=><div key={l} className="summary-row"><span className="label">{l}</span><span className="value">{v}</span></div>)}
          <hr className="summary-divider"/>
          <div className="summary-row summary-total"><span className="label" style={{fontWeight:500}}>Total</span><span className="value">R$ {Number(sel.price).toFixed(2)}</span></div>
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,background:'var(--gray-50)',borderRadius:'var(--radius-md)',padding:'12px 14px',marginBottom:20}}>
          ...
          <div>
            ...
            <div style={{fontSize:12,color:'var(--gray-500)',marginTop:2}}>
              <i className="ti ti-device-mobile" aria-hidden="true" style={{fontSize:13,verticalAlign:'-2px',marginRight:3}}/>{client?.phone}
            </div>
          </div>
        </div>
